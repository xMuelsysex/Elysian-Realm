...
async function handleRequest(request: IncomingMessage, response: ServerResponse, controller: AdminController): Promise<void> {
  ...
  if (request.method === "POST" && url.pathname === "/api/admin/llm/action-proposal") {
    const body = await readJsonBody(request);
    if (!body.ok) {
      writeError(response, 400, "INVALID_JSON", body.message);
      return;
    }
    const result = await controller.proposeLlmAction(body.value);
    writeJson(response, result.status, result.body);
    return;
  }

  if (request.method === "POST" && url.pathname === "/api/admin/llm/conversation-turn") {
    const body = await readJsonBody(request);
    if (!body.ok) {
      writeError(response, 400, "INVALID_JSON", body.message);
      return;
    }
    const result = await controller.proposeConversationTurn(body.value);
    writeJson(response, result.status, result.body);
    return;
  }
